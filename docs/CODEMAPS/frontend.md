...
| `report-table.tsx` | Shared report/table primitives: `SortHeaderButton`, `SortResizeTh<K>`, `TableFilter`, `KpiGradientBar`, and `Tile` (KPI/stat card; optional clickable `onActivate`/`activateLabel`, `trend`, `bar`). **0.177.0:** `Tile` gained an optional `hint` prop rendering an `InfoTooltip` as a SIBLING of the (clickable) tile — never nested inside the button (a tooltip trigger inside a button is a nested-interactive axe fail). **0.194.0:** `TableFilter` renders ONE clear ✕ overlaid inside its `type="search"` field — the native webkit cancel button is suppressed and `pr-8` (conditional on a value) reserves the room, because a sibling button read as two clears in Chrome/Safari and as the only one in Firefox. Used by 7 panels | Shared; carries the interaction-state atoms |
...
| `resource-workload-rows.ts` | Pure `buildResourceWorkload` engine. **★ Must receive the COMPLETE resource list:** it derives its `managed`/`nameToId` lookups from that argument alone and `resolve()` falls through to `ensureUnlinked` on a miss, so a pre-filtered list does not hide anyone — it re-surfaces their work as an *unlinked* row, whose "Clear unlinked" control deletes matching absences and shifts. Display filtering belongs on the BUILT rows (`ResourceWorkload`'s `hideExternal` prop) | No React; 0.194.0 |
...
| **Timelog (0.144.0+)** | | |
| `timelog-panel.tsx` | Timelog time-booking view (`timelog` AppView, "Time bookings"): **two-step fetch** — **Load people** pulls the directory (inactive/nameless rows filtered out) into a scrollable, client-side-filterable people↔resource matching table; tick whom to include; **Fetch bookings** then pulls timesheets for only the ticked employees (org scope). Per-row ✕ remove + multi-select bulk-remove, **Clear all**, KPI tiles, a project↔budget matching table, and the apply-to-budget confirm flow. **Load my projects** (PM-managed) + **Include closed** checkbox + customer picker bootstrap projects before any bookings exist; a blocking loading modal with **Cancel** aborts the in-flight fetch. Attribution note: a resource's hours count as booked only when its Timelog user is linked to a resource AND the booking's project to a budget bucket; PM note explains the managed-projects requirement. `isPopout` = read-only. **0.194.0:** the orchestrator delegates to `timelog-people-table.tsx` (people-matching table) and `timelog-apply-confirm.tsx` (the itemised apply confirm); apply freezes BOTH the overlay (`pendingApply`) and the budget baseline (`pendingBudgets`) at confirm-open and refuses to write if the budget changed underneath | Conditional mount; 0.144.0+ |
| `timelog-people-table.tsx` | `TimelogPeopleTable` — the people↔resource matching DataTable, extracted from the panel. Presentational: data + handlers as props | Component; 0.194.0 |
| `timelog-apply-confirm.tsx` | `TimelogApplyConfirm` — the itemised apply confirm bar (bucket · line · period · current → next). Itemised, not a count, because `actualHours` is hand-editable and applying can overwrite a typed figure | Component; 0.194.0 |
| `task-filters.ts` | Pure i18n-free `resolveEffectiveFilters(values, options)` — resolves an Open-Points assignee/group/label filter whose value no longer exists in the derived `<select>` options back to `FILTER_ALL`, so the row filter and the control cannot disagree. Each check MIRRORS how the row filter compares that field (assignee/group exact, label case-insensitive); `GROUP_NONE` is exempt (the group select renders a permanent "No group" option). The RAW state is deliberately not written back, which makes the fallback self-healing | Pure; 0.194.0 |
| `digest/digest-mail-sender.ts` | `createDigestMailSender` — resolves ONLY when Graph accepted the mail and throws on every other outcome (the hook treats resolution as "sent"). Its error text carries neither recipient nor token; it reaches the diagnostics ring | Pure factory; 0.194.0 |
