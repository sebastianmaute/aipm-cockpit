...
| `settings-sections/ai-section.tsx` | Claude API key + model selection (live picker via `useChatModels` — see AI orchestration) + session/weekly token-cap inputs + consent grant/revoke; embeds `AiUsagePanel` | |
