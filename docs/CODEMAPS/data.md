...
| `ai-rich-text.ts` | yes, browser-only | `sanitizeAiRichText` / `withAiRichFields` (`sanitizeRichHtml` / `RICH_ALLOWED_TAGS` — guards the seven rich entity fields: `Task.description` plus the six in `AI_RICH_FIELDS`) · `sanitizeAiDocumentRichText` (`sanitizeDocumentHtml` — model-authored document `paragraph.html` only). ★★ Still not interchangeable, but the delta is now **one tag**: `DOCUMENT_ALLOWED_TAGS` is `[...RICH_ALLOWED_TAGS, "img"]`, so wiring a document boundary to the rich one drops IMAGES and nothing else (`img` is VOID, so it vanishes rather than unwrapping). ★★★ This cell claimed "DROPS all nine of `s`/`code`/`pre`/`blockquote`/`hr`/`mark`/`sub`/`sup`/`img`" against the RETIRED `sanitizeTemplateHtml`; re-measured 2026-08-11 on dompurify 3.4.13, eight of those nine are in `RICH_ALLOWED_TAGS` and the old worked example `"<p>a</p><hr><p>b</p>"` is now byte-identical through both sanitizers |
