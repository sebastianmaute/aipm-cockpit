<!-- Generated: 2026-07-30 · counts re-verified 2026-08-10 at the merge with main 528dd5fe | App 0.231.0 "Sargent" | Workspace SCHEMA_VERSION = 11 | Files scanned: types.ts, workspace.ts, csv/markdown codecs, turso-schema.ts, browser-backend.ts, sanitize*, rich-text* | Token estimate: ~1100 -->
...
| `ai-rich-text.ts` | yes, browser-only | `sanitizeAiRichText` / `withAiRichFields` (`sanitizeRichHtml` / `RICH_ALLOWED_TAGS` — guards the seven rich entity fields: `Task.description` plus the six in `AI_RICH_FIELDS`) · `sanitizeAiDocumentRichText` (`sanitizeDocumentHtml` — model-authored document `paragraph.html` only). ★★ Still not interchangeable, but the delta is now **one tag**: `DOCUMENT_ALLOWED_TAGS` is `[...RICH_ALLOWED_TAGS, "img"]`, but the BEHAVIOUR delta is three things and two of them WIDEN — wiring a document boundary to the rich one drops `img` (VOID, so it vanishes rather than unwrapping), newly ADMITS arbitrary `data-*` (`sanitizeDocumentHtml` sets `ALLOW_DATA_ATTR: false`; `sanitizeRichHtml` keeps DOMPurify's default TRUE — measured: `rich('<p data-foo="1">a</p>')` keeps the attribute, `doc(...)` drops it), and cuts the cap 20 000 → 5 000, where `capHtmlText`'s truncation branch FLATTENS marks to escaped plain text. ★★★ This cell said "drops IMAGES and nothing else", which is false in the dangerous direction. It also claimed "DROPS all nine of `s`/`code`/`pre`/`blockquote`/`hr`/`mark`/`sub`/`sup`/`img`" against the RETIRED `sanitizeTemplateHtml`; re-measured 2026-08-11 on dompurify 3.4.13, eight of those nine are in `RICH_ALLOWED_TAGS` and the old worked example `"<p>a</p><hr><p>b</p>"` is now byte-identical through both sanitizers |
