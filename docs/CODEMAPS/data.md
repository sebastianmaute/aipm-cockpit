...
Role {                                       // discipline × grade combo
  ...
  disciplineId    number
  gradeId         number
  internalRate    number                     // per hour, in plan.currency — ALWAYS the
                                             //   cost-math source (periodCost/budget/EVM/reports);
                                             //   DERIVED from internalRateDay when rateBasis==="day"
  externalRate    number                     // per hour, customer-billable (same derived rule)
  internalRateDay? number                    // 0.177.0: internal day rate; authoritative when
                                             //   rateBasis==="day" (hourly then derived via workdayHours)
  externalRateDay? number                    // 0.177.0: external/billing day rate; same rule
  rateBasis?      "day" | "hour"             // 0.177.0: which unit the user edits (the other is
                                             //   auto-materialized + locked). Absent ⇒ "hour" (legacy
                                             //   roles keep their exact hourly as authoritative);
                                             //   sanitizeRole sparse-emits "day" ONLY. Day↔hour
                                             //   materialization lives in pure role-rates.ts
                                             //   (materializeRoleRates(role, workdayHours)).
                                             //   All three ride ROLES_CSV_COLUMNS + ROLES_MD_COLUMNS
                                             //   → CSV + Turso single/tenant (roles ∈ ENTITY_SPECS,
                                             //   turso-migrate self-heals the added columns)
  localModifiedAt? ISO 8601 timestamp
}
...
(**0.177.0:** sparse-emits `rateBasis: "day"` only + optional `internalRateDay`/
`externalRateDay`, so legacy hour-basis roles stay byte-identical; no cross-derive
at load — `workdayHours` is unavailable there, the editor materializes on edit),
...
## Jira multi-project sync (`jira-projects.ts`) — 0.156.0+

Pure, dependency-light helpers for multi-project Jira sync, kept out of the
lazy-loaded `jira-api.ts` so boot-time consumers (task badge, settings UI,
`use-settings.ts`) can import cheaply.

Exports:
- `jiraProjectKeyOf(issueKey: string): string` — Extract project key from issue key (e.g., `"LOP-42"` → `"LOP"`)
- `jiraProjectKeys(config): string[]` — Deduplicated union of primary + extra project keys (primary first)
- `isReadOnlyIssue(issueKey, config): boolean` — Determine if an issue is from a read-only extra project
