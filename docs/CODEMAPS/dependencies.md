<!-- Generated: 2026-07-30 · counts re-verified 2026-08-19 at 6046dcd2 | App 0.253.0 "Schroeder" | Files scanned: package.json, vitest.config.ts, playwright.config.ts, src/proxy.ts | Token estimate: ~750 -->
...
| `lucide-react` | ^1.31.0 | ONE consumer, `rich-text-toolbar.tsx`. Heroicons is still the app-wide icon set — **78** files, measured 2026-08-21 by counting the lines `grep -rln "@heroicons/react" src/app` returns, and re-measure rather than quoting this; do not reach for lucide elsewhere without deciding to switch. ★ The app-wide migration question is unowned — `docs/open-followups.md` §145 and `docs/tech-debt-register.md` TD-8 |
