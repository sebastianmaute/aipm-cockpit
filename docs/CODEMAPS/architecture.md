...
- **Budget earned value / CPI (0.197.0)** — `budget-earned-value.ts` (pure engine: bucket %complete from a manual override or linked-`taskIds` completion share, then earned value = budgeted cost × %complete) feeds `budget-report.ts`'s `earnedValue`/`costPerformanceIndex` (= EV ÷ actual cost) per bucket and an ALL-OR-NOTHING project rollup (one bucket with unknown progress blanks the whole-project figure). `costPerformanceIndexHealth` (`budget-health.ts`) bands the true 0-1 EVM ratio. Distinct from the Dashboard's task-effort-based `evm.ts` SPI/CPI below — this is bucket-level, feeds only the Budget panel's new "Cost performance (CPI)" tile, and is not folded into the Dashboard RAGs. `BudgetBucket.taskIds`/`percentComplete` persist across all six write paths (sparse-emitted).
