...
| `NEXT_PUBLIC_TIMELOG_TENANT` | Timelog tenant slug used only when Settings → Integrations has never configured Timelog (fills the empty built-in default). Unlike the two variables above, this does **not** override an already-stored tenant — the tenant is a URL path segment already sent to the Timelog API and persisted on the device, so a later change here never silently repoints an already-configured install |
