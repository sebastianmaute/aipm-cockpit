...
`calendar-chip.tsx:7` · `entity-link-picker.tsx` (the "NOT a `<button>` inside one" note on the
option row — cited by line until 2026-09-07, when a refactor stranded the number) ·
`file-picker-button.tsx:14` ·
...
| **Chips / pills** — `chat-prompt-chips.tsx:36` · `dashboard-delta-strip.tsx:81` · `milestone-horizon-strip.tsx` 50, 78 · `knowledge-panel.tsx:536` · `stakeholder-map-panel.tsx:189` · plus 2 | 8 | `rounded-full` pill | **correctly hand-rolled** | Pill-shaped, often carrying a `Dot`/`Badge` child. No primitive models a chip; `Badge` is presentational and not focusable. ★ A chip's own **remove ✕** is a different thing and is an `IconButton` candidate — `raci-chip-picker.tsx:119`, `saved-views-menu.tsx:118`, `labels-input.tsx:115` and `stakeholder-recipient-input.tsx:154` are counted there, not here. |
...
| **Drag handles** — `budget-panel.tsx` · `reports.tsx` · `dashboard-tile.tsx` · `roles-editor.tsx` (×2) | 4 | `⋮` grip, `cursor-grab` | **converted — none left hand-rolled** | ★★★ THIS ROW USED TO READ "correctly hand-rolled", and the blocker it named was real, not a rationalisation: "the `DragHandle` primitive exists but does not own the keyboard reorder these need". `DragHandle` forwarded `draggable`/`onDragStart`/`onMouseDown` ONLY, so a caller spreading `useListReorderDnd().handleProps(id)` lost `onDragEnd` (stranding the hook's `dragId` for the rest of the session, so a later drop reorders an item nobody picked up) and `onKeyDown` (the ONLY mouse-free reorder path) **silently** — a spread of a wider object is not an excess-property tsc error. The primitive now forwards all four, plus `title`, and all five reorder grips spread `handleProps` straight onto it. ★ What changed visibly, and was accepted: the hand-rolled glyph became the primitive's heroicon `⋮` — `⠿` in the budget, reports and dashboard-tile grips, `≡` in the two `roles-editor.tsx` ones (its own header comment says so) — and `<button type="button">` became `div role="button" tabIndex={0}`. Nothing depended on the native button — none of the five sits in a form, takes `disabled`, or is selected by a `button.foo` rule. ★★ Each grip's row-unique `aria-label` moved verbatim to `ariaLabel`. **No gate could have caught losing one** — axe flags no duplicate accessible name in any view at any seed size — so a multi-row unit test is the only KIND of detector there can be. All five grips are covered, but NOT all in a namesake file: `budget-panel` · `reports` · `roles-editor` (×2) in their own, and `dashboard-tile.tsx` in `dashboard-grid.test.tsx` — it has no test file of its own (`ls src/app/dashboard-tile.test.tsx` → No such file). ★★ The `4` at left is the ORIGINAL audit measurement and is deliberately NOT changed; bumping it by hand would claim a re-run of the scan that produced it. The population it counted is now zero. Re-measure the primitive's reach with `grep -rn "<DragHandle" src/app --include="*.tsx" --exclude="*.test.tsx"`, and read its OUTPUT rather than any number quoted here — one line it returns is a doc comment in `drag-handle.tsx` that quotes the same string and so matches its own grep. ★★★ TWO OF THE FOUR SITES THIS ROW NAMED WERE ALREADY WRONG BEFORE THE MIGRATION, and neither is a reorder grip — this is the row's own "one entry no longer resolves to a handle" caveat, now identified. `resource-calendar-rows.tsx` had ALREADY adopted `DragHandle` for its two absence-resize grips (decorative variant, no `ariaLabel`). `workspace-section.tsx`'s `⠿` is not a handle at all: an `aria-hidden` `pointer-events-none` `<span>` hinting that the pane corner is resizable, carrying no `draggable` and no handler. Both are correctly untouched — they do not use `useListReorderDnd` and have a different drag lifecycle. |
...
| **`×`/`✕`/`&times;` glyph closes** — `actions-panel.tsx:155` · `change-edit-modal.tsx:645` · `dashboard-tip-card.tsx:83` · `inline-ai-edit-popover.tsx:49` · `knowledge-links-field.tsx:58` · `raci-panel.tsx:224` · `raci-chip-picker.tsx:119` · `resource-edit-modal.tsx:292` · `resource-workload.tsx:318` · `timelog-people-table.tsx:142` · `saved-views-menu.tsx:118` | 11 sites | `<button>` with a bare glyph child | text glyph, no SVG | **IconButton candidate** | `IconButton` + `XMarkIcon` | These are the overlap between Part 1 and Part 2: converting the element and replacing the glyph is one edit, not two. All eleven already carry an `aria-label`, so the glyph is decorative and safe to swap — **except** where a test reads it (none of these eleven; see Part 2). ★ `roles-editor.tsx` 299 and 403 are already `IconButton` but still pass a literal `×` as the child — glyph-only work, the element is done. ★★★ **`entity-link-picker.tsx` was the twelfth and the CLAIM was false, not merely mis-numbered.** Its remove control is already `IconButton` + `XMarkIcon aria-hidden`, so there is no `<button>` and no bare glyph to convert — and it was already that shape at the merge-base this row was last touched under, so the row was stale before any refactor stranded its line number. The only `×`/`✕` left in that file are in comment prose. Reproduce: `grep -n '×\|✕\|XMarkIcon' src/app/entity-link-picker.tsx`. |
...
| `role="listbox"` | one per file; regenerate with `grep -rn 'role="listbox"' src/app --include=*.tsx \| grep -v test` | **correctly hand-rolled** | Real combobox listboxes; `combobox-shared.tsx` is the primitive and the rest are its peers with divergent option shapes. ★★ **No count and no file list is quoted here on purpose, and restoring either is a regression.** This row said "**5** lines / 5 files" and named them — after correcting an earlier "4 sites" against the same five — and the grep returned **seven** on 2026-09-07: `single-entity-picker.tsx` and `undo/undo-control.tsx` had joined and nothing moved the number. Three of the five citations had drifted too, and only one of the three was loud: `entity-link-picker.tsx` failed the range check, while `combobox-shared.tsx` was silently 1 line off and `stakeholder-recipient-input.tsx` 7. The other two were still exact — which is the point, since a list two-fifths wrong reads exactly like a list that is right. Run the grep. |
...
| `entity-link-picker.tsx` (the `aria-hidden` glyph `<span>` in the `onOpen` chip-body branch) | `↩` | `entity-link-picker.test.tsx`, the "Label-bleed guard" assertion | `chip.textContent` toContain | **keep — semantic text** — ★ re-verified 2026-09-07: the pin is live. Both cites were carried by line until then and BOTH had drifted; find them with `grep -n '↩' src/app/entity-link-picker.tsx src/app/entity-link-picker.test.tsx`. |
...
| `actions-panel.tsx` · `change-edit-modal.tsx` · `dashboard-tip-card.tsx` · `inline-ai-edit-popover.tsx` · `knowledge-links-field.tsx` · `raci-panel.tsx` · `raci-chip-picker.tsx` · `resource-edit-modal.tsx` · `resource-workload.tsx` · `timelog-people-table.tsx` · `saved-views-menu.tsx` · `history-panel.tsx` (×2) · `chat-panel.tsx` (×2) · `project-form-fields.tsx` · `raid-panel-toolbar.tsx` · `reports.tsx` · `knowledge-panel.tsx` (×2) · `roles-editor.tsx` (×2) | 155 · 645 · 83/90 · 49 · 58/64 · 224 · 119/128 · 292/297 · 318/328 · 142/149 · 118/131 · 242, 370 · 766, 789 · 691/697 · 146/152 · 558/565 · 451, 517 · 299, 403 | `×` `✕` `&times;` | close / remove / clear | **replace** | `XMarkIcon` | Every site listed already carries an `aria-label`, so the glyph is decorative and the a11y tree is unchanged by the swap. ★★ Three spellings of the same idea are in use (`×` U+00D7, `✕` U+2715, `&times;`) — the inconsistency alone is a reason to converge. `XMarkIcon` is already imported in **13** files (`grep -rl XMarkIcon src/app \| wc -l`), so this adds no dependency. ★ Those 13 do NOT overlap the files listed here — the swap adds the import to each of these, it does not merely reuse an existing one. ★★★ **`entity-link-picker.tsx` (`233/251`) was struck on 2026-09-07 — same disproof as the Part 1 `×` row above: it is already `IconButton` + `XMarkIcon`, so it was never a site.** ★★ **The site total that stood here ("~26") is deliberately gone rather than decremented, because it never reconciled with its own numbers column** — count the cells and you get materially more sites than "~26", so decrementing it by two would have produced a second wrong number wearing the authority of a correction. Count the cells, or re-derive with `grep -rn '×\|✕\|&times;' src/app --include=*.tsx`. |
...
| `raid-panel-rows.tsx` | 335 | `↩` | parent-link chip | **replace**, blocked | `ArrowUturnLeftIcon` | Decorative next to `#{pid}`. ★ **Blocked**: the sibling site in `entity-link-picker.tsx` carries the same glyph and IS test-pinned (the row above in the glyph-pin table), so converge both or neither — replacing one leaves two spellings of the same affordance. |
...
| `raid-risk-matrix.tsx` · `stakeholder-map-panel.tsx` | 43, 87 · 149, 220 | `←` `→` `↑` | axis direction labels | **keep — semantic text** | Part of the axis caption ("← Impact →"). The arrows carry the low/high direction; nothing else does. |
...
| `workspace-section.tsx` | n/a | `⠿` | corner resize HINT | **keep — not an icon** | ★★ This row used to name THREE files (`budget-panel.tsx` · `reports.tsx` · `workspace-section.tsx`) with line numbers, call all three "drag grip", and rule `keep` because "Heroicons has no grip glyph, so there is nothing to swap to". That reasoning was wrong about two of them: the budget and reports grips were reorder grips, they migrated to the shared `DragHandle` (see the Drag handles row above), and the primitive's glyph IS a heroicon — `EllipsisVerticalIcon`. ★ The survivor is not a grip and never was: an `aria-hidden` `pointer-events-none` `<span>` in the pane's bottom-right corner, with no `draggable` and no handler. `keep` still holds for it, for a DIFFERENT reason — it decorates a resize affordance rather than being one. ★ Line numbers dropped rather than renumbered: a wrong line number is a symptom, and the claim they supported has changed. Re-measure with `git grep -c "⠿" -- src` — 2 hits on 2026-08-20, this one and a doc comment in `dashboard-tile.tsx` recording the change. |

★ **Glyphs the plan's list missed entirely**, found by widening the scan: `⠿` (3 at the time of that scan, drag grips — two have since migrated to `DragHandle`; see the two rows above) ·
