...
| `role="dialog"` outside `modal.tsx` / `popover-panel.tsx` | **26** lines / **23** files (2026-09-02) | **correctly hand-rolled** | ★★★ **RE-MEASURED 2026-09-02, AND THE OLD ARITHMETIC MIXED TWO COUNTING METHODS.** It read "**24** lines / **21** files … 26 lines / 22 files total − the 2 in `modal.tsx`": the 26 came from counting lines where `role="dialog"` is the JSX ATTRIBUTE, while the 2 subtracted for `modal.tsx` counted ALL mentions there — the attribute at its render plus a COMMENT naming it in the file header. `modal.tsx` has **one** attribute line, so the subtraction was one too many. Count consistently with `git grep -nE '^\s*role="dialog"' -- 'src/**' \| wc -l` (27 today, including tests, the way this row has always counted) and `git grep -lE '^\s*role="dialog"' -- 'src/**' \| wc -l` (24 files), then subtract `modal.tsx` alone. `popover-panel.tsx` still has **none** — re-measured, not carried over. ★★ **+1 line / +1 file of this is `raci-chip-picker.tsx`, added 2026-09-02** (open-followups §334): its panel is a `PopoverPanel` and the `role` is passed as a PROP, because `ariaLabel` on a role-less `<span>` is prohibited by ARIA 1.2 and was being dropped. So a `role="dialog"` in a consumer no longer implies a hand-rolled dialog — check whether it is an attribute on the consumer's own element or a prop handed to the primitive. ★ Every other row in this document counts LINES; the "21" this row used to carry was the FILE count. Every one is a popover panel or floating window that already routes through `PopoverPanel` or owns its own drag/dismiss wiring. No hand-rolled modal was found that should be `Modal` — this was the cleanest result in the audit. |
