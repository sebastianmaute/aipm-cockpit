...
| TD-6 | Duplication ≈ 2.52% total / tsx 2.65% (gate at 3.1%) — not yet ≤50% of the Phase 1 baseline | Phase 3/4 | tech-lead | 2026-10-03 | **Progress 2026-07-03:** (1) extracted the shared `SavedViewsMenu` (select+save+delete shell across the 3 views-controls, ~133 lines) → total 2.83%→2.61%, tsx 3.24%→2.83%; gate 3.7→3.3. (2) extracted `edit-modal-chrome.tsx` (`ModalFieldError` ×3 + `StakeholderChipPicker` ×2 + `ModalEditFooter` ×2, the top cross-file edit-modal clones) → total 2.61%→**2.52%**, tsx 2.83%→**2.65%**; gate 3.3→**3.1**. Remaining top clusters (own effort): report-panel intra-file self-clones (`raid-report-panel` 28-line, `change-report-panel`), `voice-button` self-clones, the panel-toolbar/register-band families. The ≤50% roadmap target is a repo-wide halving, not single-extraction-achievable. Keep ratcheting DOWN opportunistically. |
