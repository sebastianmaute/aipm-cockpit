...
| TD-6 | Duplication ≈ 2.42% total / tsx 2.46% (gate at 2.9%) — not yet ≤50% of the Phase 1 baseline | Phase 3/4 | tech-lead | 2026-10-03 | **Progress 2026-07-03:** (1) shared `SavedViewsMenu` (3 views-controls shell) → total 2.83%→2.61%, tsx 3.24%→2.83%; gate 3.7→3.3. (2) `edit-modal-chrome.tsx` (`ModalFieldError`/`StakeholderChipPicker`/`ModalEditFooter`, top cross-file edit-modal clones) → total 2.61%→2.52%, tsx 2.83%→2.65%; gate 3.3→3.1. (3) `SortResizeTh<K>` in `report-table.tsx` (the sortable+resizable `<th>` trio) — converted **56** call sites across raid-report (34) / resources-report (16) / change-report (6) → total 2.52%→**2.42%**, tsx 2.65%→**2.46%** (tokens 3.29%→2.83%); gate 3.1→**2.9**. Remaining clusters (own effort): `raid-report-panel`/`change-report-panel`/`resources-report` intra-file BODY-row self-clones (the report `<td>` rows, not the now-shared headers), `voice-button` self-clones, panel-toolbar/register-band/notifications families, and the change-panel↔raid/stakeholder/milestones-panel toolbar families. The ≤50% roadmap target is a repo-wide halving, not single-extraction-achievable. Keep ratcheting DOWN opportunistically. |
