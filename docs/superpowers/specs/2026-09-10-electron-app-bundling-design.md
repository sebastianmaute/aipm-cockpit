...
**An automatic version CHECK is not refused; it simply has nowhere to point.** The argument above turns entirely on a payload that gets executed, and does not carry over. An endpoint serving only a version *string*, which the app compares against its own and answers with the same open-the-Releases-page affordance, downloads nothing, runs nothing, needs no credential if the endpoint is public, and at worst — if compromised — nags when there is nothing to get, or suppresses the nag when there is. That is a materially smaller exposure than the update channel and would be a real improvement on a menu item the user has to remember to click.

What blocks it today is only that **no such endpoint exists, and standing one up is a separate decision nobody has taken.** Measured 2026-09-12, not assumed: `.gitlab-ci.yml` has no `pages:` job (`grep -n "pages:" .gitlab-ci.yml` → no match) and `desktop/electron-builder.yml` has no `publish:` key (`grep -n publish desktop/electron-builder.yml` → no match), so there is neither a static site nor a publish target to write a version to. Picking a host, deciding what writes to it on each tag, and deciding what the app does when it cannot reach it are all still open; the manual item is what ships in the meantime, not a verdict against the check.
...
**Open question 1 stays open.** Choosing a manual route answers nothing about whether `electron-updater`'s generic provider works over UNC versus an internal HTTPS feed; that spike is still owed before any automatic path is committed to.
