...
**Goal:** Make Petrol and Mockup first-class **built-in color schemes** so the app look is fully scheme-driven, with NO hardcoded `data-style` CSS blocks as the source of truth. Extend the scheme model to carry the structural (non-color) tokens Mockup needs.

**Context:** Phase 1 (v0.182.0 "Reynolds") built the scheme engine — light+dark maps, built-in dark-capable schemes (Harbor/Meridian/Umber), no-flash boot, unified Settings → Appearance picker, WCAG-AA derivation. Petrol and Mockup were left as `globals.css` `data-style` blocks. Phase 2 finishes the original ask ("move brand tokens to a color scheme … remove that it is hardcoded into the app") by folding Petrol + Mockup into the same scheme mechanism.

**Decisions (confirmed with user):**
- **Scope:** Petrol **and** Mockup both become built-in schemes; the `data-style` axis collapses to the single constant `"custom"` (everything is scheme-driven).
...
### One runtime path
Today: `data-style ∈ {petrol, mockup, custom}` selects a `globals.css` CSS block (Petrol `:root`, Petrol-dark `:root.dark`, Mockup `:root[data-style="mockup"]`) OR the inline scheme overrides (`custom`). After Phase 2 there is exactly ONE path — the **scheme store**. `data-style` is pinned to `"custom"`; Petrol/Mockup are scheme ids in the store (like Harbor).

### `globals.css` after Phase 2
- `:root` **KEEPS** the Petrol-light values (colors + the 7 structural tokens) as the **static no-JS / pre-boot fallback baseline** — the one unavoidable "hardcoded" spot (needed for the split-second before the boot IIFE runs and for JS-disabled clients). It is now a *fallback*, not the source of truth; the Petrol scheme re-writes identical values inline when Petrol is active (a no-op visually).
- **REMOVE** the `:root.dark` **token-override** block. Petrol-dark now rides the Petrol scheme's dark map (applied inline). `.dark` **remains a class toggle** — hundreds of Tailwind `dark:` utilities depend on it; only the CSS-variable redefinitions inside the `.dark` block move into the scheme.
...
|------|--------------------------|
| `src/app/globals.css` | Keep `:root` Petrol-light fallback (colors + structural); DELETE `.dark` token block + `[data-style="mockup"]` block; keep `.dark` class. |
...
| `src/app/scheme-tokens.ts` | Add `STRUCTURAL_TOKENS` registry + `PETROL_STRUCTURAL` / `MOCKUP_STRUCTURAL` seed maps; `resolveSchemeColors` unchanged (colors only — structural is not AA-derived). |
| `src/app/builtin-schemes.ts` | Add `PETROL` + `MOCKUP` `ColorScheme`s (with `structural`); `BUILTIN_SCHEMES` now 5; `resolveActiveScheme` unchanged; add `resolveActiveStructural(store)`. |
...
| `src/app/use-style.tsx` | `syncScheme` also resolves + applies + mirrors the structural map; one-time migration of `lop-style` `petrol`/`mockup` → activeId; always write `data-style="custom"`. |
| `src/app/use-theme.tsx` | `apply()` pin-light from `data-scheme-dark` only (mockup special-case gone). |
| `src/app/settings-sections/appearance-section.tsx` | Petrol/Mockup are scheme options (already in the unified `<select>`); remove any residual Petrol/Mockup style-toggle; both read-only in editor (built-in). |
| `src/app/color-scheme-editor.tsx` | Petrol/Mockup read-only (built-in) — no new UI. |
| `e2e/a11y.spec.ts` | Re-seed the 5 axe combos as scheme selections (petrol/mockup/harbor × theme) instead of `data-style`. |
| palette-guard test(s) | Allowlist `builtin-schemes.ts` for the raw `shadow`/`gradient` strings it now contains. |
| `AGENTS.md` + memory | Update the "Scheme-driven color schemes" bullet (Petrol/Mockup now schemes; data-style constant; structural token group). |
...
- **Petrol structural:** `--shadow-*: none`, `--gradient-kpi: var(--ui-green)`, `--delta-chip-pad: 0`, `--rag-green-chip/-red-chip: transparent`.
...
- `lop-style === "petrol"` → `activeId = "petrol"`
...
1. **Petrol-dark AA re-verify.** Petrol-dark currently uses hand-tuned `-strong` values + a sparse override + Tailwind `dark:` utilities. As a scheme it runs through `deriveAaVariants` (recompute `-strong` vs `--surface-muted`) → values may differ. The axe gate (Petrol light+dark in `A11Y_VIEWS`) catches regressions; tune the Petrol dark map like Phase 1 tuned Harbor. Brand hues (`--ui-green` etc.) are theme-invariant so the Petrol-dark map can be sparse (surface/surface-muted/foreground + let derivation handle `-strong`), relying on the `:root` fallback for the rest — but VERIFY the derived `-strong` passes before shipping.
...
5. **No-JS fallback.** Keeping `:root` = Petrol-light means a JS-off client shows Petrol (not Harbor). Acceptable + documented (Harbor-default needs JS; the pre-paint boot writes it before first paint for JS-on clients).

---

## Testing strategy

- **Unit:** structural validator (accept safe / reject `url(`, `;`, `{`); Petrol & Mockup scheme resolve to expected maps; migration maps `petrol`/`mockup`→activeId + sets `custom`; `effectiveDark` new signature; `resolveActiveStructural`.
...
- **Component:** AppearanceSection selecting Petrol/Mockup applies the scheme + structural; editor read-only for both.
- **e2e/axe:** 5 combos green (petrol-light, petrol-dark, mockup-light, harbor-light, harbor-dark).
