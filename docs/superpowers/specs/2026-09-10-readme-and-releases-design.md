...
| GitLab  (GitLab) is `visibility: internal`, `releases_access_level: enabled`, **`packages_enabled: false`** | `glab api example-group/aipm-cockpit` |
...
| GitHub mirror is a **private, personal-account** copy of `main`, `only_protected_branches: true`, with **zero tags mirrored** | `glab api example-group/aipm-cockpit/remote_mirrors`, `gh api .../tags` |
