...
Migrate `src/app/raid-panel.tsx` (1345 LOC) to the brand palette + surface tokens with deliberate per-system color decisions for the three semantic systems, plus standard chrome + stale/error-box mapping. No behavior/markup change.
...
|---|---|---|
| **R** Risk | `bg-red-100 text-red-800 dark:bg-red-950/40 dark:text-red-300` | `bg-ui-pink/15 text-ui-pink dark:bg-ui-pink/20` |
| **A** Action | `bg-sky-100 text-sky-800 dark:bg-sky-950/40 dark:text-sky-300` | `bg-ui-blue/15 text-ui-blue dark:bg-ui-blue/20` |
| **I** Issue | `bg-amber-100 text-amber-800 dark:bg-amber-950/40 dark:text-amber-300` | `bg-ui-purple/15 text-ui-purple dark:bg-ui-purple/20` |
| **D** Decision | `bg-violet-100 text-violet-800 dark:bg-violet-950/40 dark:text-violet-300` | `bg-ui-green/15 text-ui-green dark:bg-ui-green/20` |
...
| Level | Current | → New |
|---|---|---|
| Low | `bg-emerald-200 hover:bg-emerald-300 dark:bg-emerald-900/50 dark:hover:bg-emerald-900` | `bg-ui-green/20 hover:bg-ui-green/30 dark:bg-ui-green/20 dark:hover:bg-ui-green/30` |
| Medium | `bg-amber-200 hover:bg-amber-300 dark:bg-amber-900/50 dark:hover:bg-amber-900` | `bg-ui-blue/20 hover:bg-ui-blue/30 dark:bg-ui-blue/20 dark:hover:bg-ui-blue/30` |
| High | `bg-orange-300 hover:bg-orange-400 dark:bg-orange-900/60 dark:hover:bg-orange-900` | `bg-ui-purple/25 hover:bg-ui-purple/35 dark:bg-ui-purple/25 dark:hover:bg-ui-purple/35` |
| Critical | `bg-red-400 hover:bg-red-500 dark:bg-red-900/70 dark:hover:bg-red-900` | `bg-ui-pink/30 hover:bg-ui-pink/40 dark:bg-ui-pink/30 dark:hover:bg-ui-pink/40` |
...
| Key | Current | → New |
|---|---|---|
| R | `bg-red-500` | `bg-ui-pink` |
| A | `bg-amber-500` | `bg-ui-purple` |
| G | `bg-emerald-500` | `bg-ui-green` |

### Stale / amber chips (~lines 381, 576, 1201)

Amber chips signalling staleness/aging items → `ui-purple` per the standard amber→purple rule:
- Border / bg-50 / text-800 pattern → `border-ui-purple/40 bg-ui-purple/10 text-ui-purple dark:border-ui-purple/50 dark:bg-ui-purple/15`
- Inline `bg-amber-100 text-amber-800 dark:bg-amber-950/40 dark:text-amber-300` chip → `bg-ui-purple/15 text-ui-purple dark:bg-ui-purple/20`
- Italic amber text (~line 881) `text-ui-purple` (single class, no separate dark variant needed)

### Red error box (~line 1214)

`rounded-md bg-red-50 px-3 py-2 text-sm text-red-700 dark:bg-red-950/40 dark:text-red-300` → `rounded-md bg-ui-pink/10 px-3 py-2 text-sm text-ui-pink dark:bg-ui-pink/15` (the canonical soft-error-box recipe from chunk 3).

## Chrome (standard sweep mapping)

The whole-chunk pattern (per `docs/DESIGN-TOKENS.md`): `zinc-*`/`border-ui-light-grey`/`divide-ui-light-grey`/`bg-ui-light-grey` → `bg-surface`/`bg-surface-muted`/`border-line`/`divide-line`/`text-foreground`/`text-muted-foreground`; `shadow-*` removed; `focus:ring-ui-dark-blue` → `focus:ring-ui-green`; `hover:bg-zinc-*` + `hover:bg-ui-light-grey` → `hover:bg-surface-muted`. KEEP `bg-ui-dark-blue text-white` fills and `text-ui-dark-blue dark:text-ui-light-grey` heading pairs.
...
- **Multiple amber chips in different contexts:** all map to ui-purple (warning/stale), giving stale items a consistent purple thread across the panel.
