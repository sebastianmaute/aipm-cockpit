...
- **Palette**: only brand tokens; RAG-semantic colors via `--rag-*` tokens; no raw shadow/gradient except via `--shadow-*`/`--gradient-kpi`.
...
- `calendar-sync-controls.tsx` (raid/changes/absence) currently `border-ui-dark-blue text-ui-dark-blue`; milestone uses `border-line bg-surface text-foreground` with leading icon. Change `CalendarSyncControls` to the milestone neutral style + leading icon so all four match.
...
- **Workload (resource planner/capacity) absence colors**: extract the calendar's `absenceCellBg`/`absenceGlyph` color+glyph map into a shared `absence-style.ts` (vacation=ui-blue/30, sick=ui-pink/30, training=ui-purple/30, other=ui-medium-grey/45 + legend). Apply in the resource planner/capacity view for upcoming absences (color the absence cells/markers by type + a legend). Reuse, don't re-derive.
