# Release B — `brand-*` → `ui-*` Palette Token Rename (Design)

**Date:** 2026-07-18
**Program:** DS sprawl audit, task 38 (Release B). Release A (0.190.22 "Pinsker", MR!281) already
de-privileged Petrol/Mockup into pluggable importable themes. Release B strips the brand word from the
load-bearing palette **token names** now that Petrol is just one theme among many.

## Goal

Rename the 12 base palette tokens from the `brand-*` prefix to a neutral `ui-*` prefix, everywhere they
...
- **Naming scheme:** neutral-prefix swap — `brand-` → `ui-`, keep the color-word suffix. Near-1:1 codemod,
...
- **Prefix word:** `ui-`.
- **Docs scope:** EXHAUSTIVE — every `brand-<color>` token string in AGENTS.md, code comments, and repo
  docs (CHANGELOG/README) becomes `ui-*`, historical prose included, plus a short supersede note.
- **Migration:** NONE (consistent with Release A "no active users"). Stored/exported `--brand-*` scheme
...
```
brand-dark-blue      brand-green         brand-green-strong
brand-pink           brand-pink-strong   brand-purple
brand-purple-strong  brand-blue          brand-white
brand-dark-grey      brand-light-grey    brand-medium-grey
```

Each appears in three coupled forms:

1. **Tailwind utility class** — `bg-brand-green`, `text-brand-dark-blue`, `ring-brand-green`, arbitrary
   `[var(--brand-green)]`, etc.
2. **CSS variable name** — `--brand-green` (in `globals.css :root`, scheme resolve output, boot keys,
   `public/themes/*.json` keys, scheme token registries).
3. **`@theme` map entry** — `--color-brand-green` (the Tailwind v4 declaration that generates the utility).

## Scope

**Rename (brand-<color> → ui-<color>):**

- All source `.tsx`/`.ts` className strings and `var(--brand-*)` references (~1637 refs / ~235 files).
- `src/app/globals.css`: `:root` var definitions, the `@theme` `--color-brand-*` map, and every internal
  `var(--brand-*)` reference.
- Scheme system: `scheme-tokens.ts` (`CORE_TOKENS`/`ADVANCED_TOKENS` token ids), `color-schemes.ts`
  (`DERIVED_TOKENS`, `VALID_TOKENS`), `scheme-apply.ts` (`STRUCTURAL_TOKENS` — none are brand-color, verify),
  boot key payloads in `boot-theme-script.ts`, and the resolved-map var names.
- `public/themes/petrol.json` + `mockup.json`: rewrite every color-map KEY `--brand-*` → `--ui-*` (values
  unchanged; the theme's *display name* "Petrol"/"Dashboard" and file path stay).
- Palette guards: `shell-palette-guard` + `palette-chrome-sweep` allowed-palette literals.
- Every test asserting `brand-<color>` classes/tokens.
...
- The brand name (company/brand name in UI text, help content, the Petrol theme display name).
- The employer name and its host domain (GitLab host, emails, CI provenance).
- `public/themes/petrol.json` filename and gallery entry `{ id: "petrol", name: "Petrol" }` — this is the brand
  *theme* identity, still called Petrol.
- `brand-logo` / `brand-icon` (asset class/id names in `app-header.tsx`, `project-empty-state.tsx`,
  `manifest.webmanifest`) — not color tokens; auto-excluded because the codemod anchors to the 12 color
  suffixes.

## Codemod strategy

Anchor every replacement to the exact 12 color suffixes with a word boundary so non-color `brand-*`
strings (`brand-logo`, `brand-icon`) and unrelated tokens (the brand name, the employer name) can never match.
...
replace `brand-(dark-blue|green-strong|green|pink-strong|pink|purple-strong|purple|blue|white|dark-grey|
light-grey|medium-grey)` → `ui-\1`. This single pattern covers `bg-brand-green`, `--brand-green`,
`--color-brand-green` (the `color-` prefix survives untouched), and `[var(--brand-green)]` alike, because
the match starts at `brand-`.
...
remaining `brand-` strings are `brand-logo`, `brand-icon`, `petrol.json`, the employer host and name, and the
capitalised brand display strings.
...
- `npm run build` (globals.css must compile — a stray `--brand-*` in a `var()` with no definition yields a
...
- Post-run residual grep: no unexpected `brand-<color>` remains.
...
- **Longest-first alternation** is mandatory or `green-strong` → `ui-green-strong` breaks into
  `ui-green`-`strong`.
- **`--color-` prefix** in the `@theme` map must survive — the pattern starts at `brand-`, so `--color-brand-`
...
- **Protected collisions.** The suffix anchor protects the brand name/the employer name/`brand-logo`; the post-run grep
  confirms it.
