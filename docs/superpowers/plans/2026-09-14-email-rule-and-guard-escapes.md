...
- Commits: write the message with the Write tool to `$LOG/msg-tN.txt`, then `git add <paths> && git commit --only <paths> -F "$LOG/msg-tN.txt"`. Never `git add -A` / `git add .`, never `--amend`, no `git stash`, no `git checkout --`. Every message ends with the line the session trailer.
