...
- Must stay green unchanged: `resource-edit-modal.test.tsx` (its delimiter cases still show `resourceErrorEmailDelimiter`), `use-chat-dispatcher.test.tsx` §422 cases (match `/emails/`), `plan.test.ts`, `plan.sanitizer-parity.test.ts`.
- The second grep lists tests that pass `emails` members without `@`. For each: if it is a NEW write through a boundary, MIGRATE it to a real address; if it only seeds stored state, no change. Record the labels in the commit body.
