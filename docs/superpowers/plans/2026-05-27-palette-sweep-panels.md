...
**Goal:** Migrate 5 Resources/Budget panel components from `zinc-*`/shadows/off-palette status colors to the semantic surface tokens + brand palette in `docs/DESIGN-TOKENS.md`, with no behavior/layout/markup changes.
...
| `focus:ring-ui-dark-blue`, `focus:ring-zinc-*`, `focus-visible:ring-ui-dark-blue` | `focus:ring-ui-green` / `focus-visible:ring-ui-green` |
| `hover:bg-zinc-50/100` + paired `dark:hover:bg-zinc-800` (and `hover:bg-ui-light-grey`) | `hover:bg-surface-muted` (single class, both modes) |
| `bg-gradient-*`, `from-*`, `via-*`, `to-*` | remove |

Leave UNCHANGED: solid fills `bg-ui-dark-blue text-white`; existing `--ui-*` utilities (`text-ui-dark-grey`, `text-ui-dark-blue`, `text-ui-medium-grey`, `text-ui-light-grey`, `border-ui-dark-blue`, `bg-ui-pink`, etc.); all non-class code (logic, markup, props, hooks).
...
- [ ] **Step 1: Migrate** — READ `src/app/resource-workload.tsx` and apply the shared mapping table (sticky `<thead>` `bg-zinc-50 … shadow-sm dark:bg-zinc-900` → `bg-surface-muted`; the name/shift buttons' `shadow-sm hover:bg-zinc-50 dark:hover:bg-zinc-800` → `hover:bg-surface-muted` minus shadow; `focus-visible:ring-ui-dark-blue` → `focus-visible:ring-ui-green`; `border-zinc-*`/`text-zinc-*` per the table).
- [ ] **Step 2: Named status edit** — the overdue-count class `font-medium text-red-600 dark:text-red-400` (appears twice) → `font-medium text-ui-pink`.
...
- [ ] **Step 1: Migrate** — READ `src/app/budget-panel.tsx` and apply the shared mapping table (≈21 `zinc-*`, the `dark:bg-zinc-900 dark:hover:bg-zinc-800` secondary button, `shadow-sm`, etc.). The primary `bg-ui-dark-blue` button stays; the secondary outline button `border-ui-dark-blue bg-white … hover:bg-ui-light-grey dark:bg-zinc-900 dark:hover:bg-zinc-800` → `border-ui-dark-blue bg-surface … hover:bg-surface-muted` (drop shadow, drop zinc).
- [ ] **Step 2: Named status edit** — the CCI tone helper line `const tone = value.amount >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-ui-pink";` → `const tone = value.amount >= 0 ? "text-ui-green" : "text-ui-pink";`.
...
- [ ] **Step 2: Named status edit** — the absence-override `<input>` class `border-amber-200 … text-amber-700 dark:border-amber-900/50 dark:bg-zinc-900 dark:text-amber-400` → `border-ui-purple/40 … text-ui-purple dark:border-ui-purple/50 dark:bg-surface dark:text-ui-purple` (amber/warning → purple; the `dark:bg-zinc-900` → `dark:bg-surface`).
...
// planning, budget) onto the brand design tokens — surface/line/muted tokens,
...
**Type consistency:** Only Tailwind utility class names are involved; the target utilities (`bg-surface`, `bg-surface-muted`, `border-line`, `text-foreground`, `text-muted-foreground`, `ring-ui-green`, `text-ui-pink`, `text-ui-green`, `text-ui-purple`) all exist (defined in E0 / already in the palette). No code symbols change. The grep regex deliberately excludes `ui-*` tokens (no digit after the color word) so it won't false-flag the targets.
