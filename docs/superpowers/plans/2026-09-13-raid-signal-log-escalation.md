...
20. **CONTROLLER RULING 2026-09-13 — undoing an AI escalation keeps its note.**
    ...
    - **SUPERSEDED 2026-09-13 (user: "fix all").** `escalateRaid` now captures a field patch (`captureFieldPart` over `escalations`, `noteLog` and, when raised, `severity`), built by `appendPatch` (`src/app/undo/append-patch.ts`) so undo removes only the escalation's own note and keeps every earlier or later note. The same `use-chat-dispatcher.undo.test.tsx` site now pins the note as REMOVED. Commits 684782a3 and fdd3b2ea.
