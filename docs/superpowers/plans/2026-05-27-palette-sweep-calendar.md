...
**Goal:** Migrate `resource-calendar.tsx` to the brand palette + surface tokens — replace `zinc`/`shadow`/Tailwind status colors and the broken `brand-light-blue`, with a decided per-state color scheme. No behavior/markup change.
...
- [ ] **Step 1: Absence-cell colors** — in `absenceCellBg()`:
  - `bg-blue-200 hover:bg-blue-300 dark:bg-blue-900/70 dark:hover:bg-blue-800/70` → `bg-ui-blue/30 hover:bg-ui-blue/40 dark:bg-ui-blue/25 dark:hover:bg-ui-blue/35`
  - `bg-red-200 hover:bg-red-300 dark:bg-red-900/70 dark:hover:bg-red-800/70` → `bg-ui-pink/30 hover:bg-ui-pink/40 dark:bg-ui-pink/25 dark:hover:bg-ui-pink/35`
  - `bg-amber-200 hover:bg-amber-300 dark:bg-amber-900/70 dark:hover:bg-amber-800/70` → `bg-ui-purple/30 hover:bg-ui-purple/40 dark:bg-ui-purple/25 dark:hover:bg-ui-purple/35`
  - `bg-zinc-300 hover:bg-zinc-400 dark:bg-zinc-600 dark:hover:bg-zinc-500` → `bg-ui-medium-grey/45 hover:bg-ui-medium-grey/55 dark:bg-ui-medium-grey/35 dark:hover:bg-ui-medium-grey/45`
...
- [ ] **Step 2: Header day-cell ternary + bases**:
  ...
  - `bg-brand-light-blue/30 text-ui-dark-blue dark:bg-ui-dark-blue/30 dark:text-brand-light-blue` → `bg-ui-green/20 text-ui-dark-blue dark:bg-ui-green/20 dark:text-ui-light-grey`
  - `bg-purple-50 text-purple-700 dark:bg-purple-950/30 dark:text-purple-300` → `bg-ui-purple/15 text-ui-purple dark:bg-ui-purple/20 dark:text-ui-purple`
...
- [ ] **Step 4: Sticky assignee `<td>` + name button**:
  ...
  - `rounded-md border border-transparent px-2 py-0.5 text-left font-medium text-ui-dark-grey shadow-sm hover:border-ui-dark-blue hover:bg-zinc-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-ui-dark-blue dark:text-ui-light-grey dark:hover:bg-zinc-800` → `rounded-md border border-transparent px-2 py-0.5 text-left font-medium text-foreground hover:border-ui-dark-blue hover:bg-surface-muted focus:outline-none focus-visible:ring-2 focus-visible:ring-ui-green`

- [ ] **Step 5: Body-cell `baseBg` ternary + cell `<td>` + button + glyph**:
  - `bg-brand-light-blue/20 hover:bg-brand-light-blue/40 dark:bg-ui-dark-blue/20 dark:hover:bg-ui-dark-blue/40` → `bg-ui-green/15 hover:bg-ui-green/25 dark:bg-ui-green/15 dark:hover:bg-ui-green/25`
  - `bg-purple-50 hover:bg-purple-100 dark:bg-purple-950/20 dark:hover:bg-purple-950/40` → `bg-ui-purple/10 hover:bg-ui-purple/20 dark:bg-ui-purple/15 dark:hover:bg-ui-purple/25`
  - `bg-zinc-50 hover:bg-zinc-100 dark:bg-zinc-900/50 dark:hover:bg-zinc-800/50` → `bg-surface-muted hover:bg-ui-medium-grey/20 dark:hover:bg-ui-medium-grey/20`
  ...
  - `focus:ring-1 focus:ring-inset focus:ring-ui-dark-blue` → `focus:ring-1 focus:ring-inset focus:ring-ui-green` (inside the cell button's template-literal className)
  - the glyph `<span>`: `text-ui-dark-grey dark:text-ui-light-grey` → `text-foreground`

- [ ] **Step 6: Legend container + chips + LegendChip border**:
  - `flex flex-wrap items-center gap-x-4 gap-y-1 px-1 text-[11px] text-ui-medium-grey` → `flex flex-wrap items-center gap-x-4 gap-y-1 px-1 text-[11px] text-muted-foreground`
  - `bg-blue-200 dark:bg-blue-900/70` → `bg-ui-blue/30 dark:bg-ui-blue/25`
  - `bg-red-200 dark:bg-red-900/70` → `bg-ui-pink/30 dark:bg-ui-pink/25`
  - `bg-amber-200 dark:bg-amber-900/70` → `bg-ui-purple/30 dark:bg-ui-purple/25`
  - `bg-zinc-300 dark:bg-zinc-600` → `bg-ui-medium-grey/45 dark:bg-ui-medium-grey/35`
  - `bg-brand-light-blue/30 dark:bg-ui-dark-blue/30` → `bg-ui-green/20 dark:bg-ui-green/20`
  - `bg-purple-100 dark:bg-purple-950/30` → `bg-ui-purple/15 dark:bg-ui-purple/20`
...
- [ ] **Step 7: Verify** (all must pass):
  1. Grep `src/app/resource-calendar.tsx` → ZERO: `zinc-|shadow-|brand-light-blue|ui-dark-blue/|bg-gradient|(amber|red|emerald|sky|rose|slate|gray|orange|yellow|teal|cyan|indigo|violet|fuchsia|green|blue|purple|pink)-[0-9]`
     (Note: `bg-ui-green/20`, `bg-ui-purple/15`, `text-ui-dark-blue`, `border-ui-dark-blue` do NOT match — no digit follows the color word, and the `ui-dark-blue/NN` backgrounds were all in the removed today-token. Confirm no `ui-dark-blue/NN` background remains.)
...
git commit -m "style(palette): resource-calendar to brand palette (today=green, holiday=purple, fix brand-light-blue)"
```
...
```ts
// 0.15.3 sweeps the resource calendar onto the brand palette: absence cells in
// blue/pink/purple/grey + glyph, today=green wash, holiday=purple wash,
// weekend=muted; fixes the previously-undefined brand-light-blue token.
```
...
Dispatch a final reviewer over `git diff main...HEAD`. Read `version.ts`/`CHANGELOG.md` directly (don't infer version from diff sign). Confirm: `APP_VERSION === "0.15.3"`, no new highlight key, real `[0.15.3]` entry, DESIGN-TOKENS updated (calendar ✅ + Calendar-status-colors section). Run the Step-7 grep on `resource-calendar.tsx` → ZERO matches (no `zinc-`/`shadow-`/`brand-light-blue`/`ui-dark-blue/NN`/off-palette-shade). Duplicate-utility scan clean. Confirm the scheme: absence cells blue/pink/purple/grey, today=green, holiday=purple, weekend=surface-muted, glyph=`text-foreground`, focus rings `ui-green`. Confirm class-strings only (no logic/markup/behavior change) and scope = `resource-calendar.tsx` + version.ts + CHANGELOG.md + docs/DESIGN-TOKENS.md (4 files). Full suite green. Then use `superpowers:finishing-a-development-branch`.

---

## Self-Review (author)

**Spec coverage:** absence-type colors (blue/pink/purple/grey) → Task 1 Step 1 + legend Step 6; today=green (fixes brand-light-blue) → Steps 2,5,6; holiday=purple wash → Steps 2,5,6; weekend=surface-muted, normal=surface → Steps 2,5; chrome (borders/header/assignee button/glyph) → Steps 2–6; release + DESIGN-TOKENS calendar section → Task 2. All spec items covered.
...
**Type consistency:** Only Tailwind class names change; targets (`bg-ui-blue/30`, `bg-ui-green/15`, `bg-ui-purple/10`, `bg-ui-medium-grey/45`, `border-line`, `bg-surface`, `bg-surface-muted`, `text-foreground`, `text-muted-foreground`, `ring-ui-green`, `text-ui-purple`) all resolve (palette `--ui-*` supports arbitrary alpha; surface tokens defined in E0). The Step-7 grep regex excludes `ui-*` alpha utilities (no digit after the color word) so it won't false-flag the new classes.
