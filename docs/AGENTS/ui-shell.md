...
### UI shell — dismissal: Escape & Tab ownership
  ...
  *while the root is there* — that much of the old claim held); dropping the
  ...
  that commits a draft on blur (`budget-bucket-modal.tsx` and `change-edit-modal.tsx` are full of them;
  ★ NOT either live STORAGE target — the Turso URL/token and the SharePoint file URL now commit only on
  an explicit Apply, so for those Escape correctly DISCARDS the draft, see `docs/AGENTS/platform.md`)
  got no reliable React `onBlur` when the dialog unmounted under it — Firefox
  ...
  and ✕ need nothing: their mousedown already moves focus. ★ Only `Modal` does this — a surface
  closing through `useDismissable` or `useFocusTrap` with a blur-commit field inside does NOT.
