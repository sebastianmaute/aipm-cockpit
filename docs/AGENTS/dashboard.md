...
no `taskIds` at all, or every linked task deleted). Its snapshot input (`bucketProgressSeries`) is Turso-only, so a
file-mode project sees every hand-entered bucket as partial for its whole span.
...
- **Coaching CTAs (first-open story):** pure i18n-free `dashboard-coaching.ts`
  ...
  is a react-hooks/exhaustive-deps WARNING. ★★★ IT IS FATAL, AND THIS LINE HAS NOW BEEN WRONG IN BOTH DIRECTIONS — do not
  trust the next revision of it either. The rule is severity 1, which did NOT fail the build while
  `npm run lint` was bare `eslint`; §214 changed that script to `eslint --max-warnings=0`, so all 25
  severity-1 rules now fail it. Check both halves rather than this sentence: `npx eslint
  --print-config src/app/icons.ts` for the severity, and `node -e "console.log(require('./package.json').scripts.lint)"`
  for the flag. Fix it anyway — HOIST it to a scalar local (`const milestoneCount =
