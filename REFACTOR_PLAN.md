...
5. **Batch E — S1/S2** pending the owner's BYO-key decision. ✅ **DONE** (v0.60.5, branch `refactor-batch-e-byok-docs`, docs-only per the BYO-key decision): consent-screen threat note, FieldNotice on all three credential fields, README "Security Model" section. S2 leak sweep verified CLEAN: no credentials in exports (CSV/MD/JSON), activity log, or console output.
