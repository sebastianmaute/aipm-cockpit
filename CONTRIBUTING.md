...
| `npm run e2e:visual` | Playwright visual-regression snapshots (opt-in; baselines are per-platform — generate CI's in the Linux container). Pins its own frozen clock (VISUAL_FROZEN_NOW in e2e/visual.spec.ts) so a FROZEN_NOW bump in e2e/seed.ts does not force a re-baseline (§573) |
...
| `npm run followups:github:check` | Compare docs/open-followups.md's Work item lines with the open GitHub issues in both directions — weekly in scheduled.yml (register-sync, report only); skips unless REGISTER_TRACKER=github |
...
| `npm run leaks:messages:check` | Fail if a commit message (or an annotated tag message on a commit) in the given range carries an identifier from the LEAK_LIST_FILE list or an assistant trailer; pass the range after --, e.g. npm run leaks:messages:check -- origin/main..HEAD. Prints only short SHAs, classes and counts. Exit 1 = leak or trailer, exit 2 = could not scan (list unset, missing or empty; no range or a bad one; a failing git call); an empty range is exit 0. Runs in CI's static job; not in gate:local, which has no commit range |
| `npm run issues:import` | Plan (--plan), apply (--apply/--resume) or close on GitLab (--close-gitlab) the import of the register's open GitLab issues into a fresh GitHub repository with their numbers preserved — runs in NO CI job; used once, at the visibility flip |
...
| `npm run release:publish` | Create and publish the GitHub Release for a tag from a verified release directory (the release workflow's publish job; needs gh, GITHUB_REPOSITORY and GH_TOKEN). Usage: npm run release:publish -- <dir> <tag>. Drafts, uploads the installer, blockmap and latest.yml, then publishes; a leftover draft is deleted and recreated. Exit 0 = published or already identical; 1 = REFUSED, a published release differs (publish a new version); 2 = could not run or confirm, safe to re-run. |
...
- Timing guards (a test that fails when code turns quadratic or backtracks) use
  ...
  — a fixed ceiling fails a correct build on a loaded machine. It judges the
  median of the per-pair ratios (each small run and the large run right after
  it), so a load step or spike spoils at most one pair. It requires an
  ...
  it returns. Pick `n` so calibration lands at ≤ 1/16 of the loop cap,
  `MAX_LOOPS` in `src/test/scaling.ts` — a site nearer the cap throws "below
  the ... floor" on a faster machine.
