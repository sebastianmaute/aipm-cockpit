...
├── task-manager.tsx      — orchestrator (~3.5k LoC); owns most client state
...
npm run test:coverage  # with v8 coverage; fails below 80%
```

Coverage is still ramping up — the 80% threshold will fail until more tests
land. Either keep adding tests or temporarily lower the threshold in
`vitest.config.ts` while bootstrapping.
