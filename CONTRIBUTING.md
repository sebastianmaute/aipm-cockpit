...
| `npm run followups:workitems:check` | Fail if an OPEN docs/open-followups.md entry lacks exactly one conforming `**Work item:**` line, a closed entry carries one, or one GitLab issue is claimed by two entries (BLOCKING; exit 1 = drift, exit 2 = the gate could not scan at all) |
