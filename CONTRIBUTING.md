...
| `npm run size:check` | Fail if a src file exceeds the `LIMIT` in `scripts/check-file-sizes.mjs` (1600 since 2026-09-03, doubled from 800 along with every baseline entry) or grows past its baselined size (ratchet). The gate is a backstop, not a target — the 200-400 line guidance below still stands. `--update` discards the doubling; re-double by hand |
