...
const enUS = {
  ...
  versionHighlight0224: "Undo steps back through more than one change at a time. The caret beside Undo used to preview a single entry; it now opens the whole history, newest first, and moving through the list marks every change back to the one you are on, with a count of how many will be reverted. Choosing an entry reverts them together as one step - one line in the activity log, one message, and one Redo to put them all back. Every button that starts an AI call now reads Stop while that call is running and cancels it, so a request you did not mean to send stops from the same button that sent it - the recommendation run that happens on its own in the background is not covered yet. In Budget, a role line inside a bucket expands to the people behind it with their booked and planned hours; booked comes from this device's Timelog cache, so it can differ from the role's own actuals and the people shown need not add up to them. And the settings list no longer wraps into a tall stack on a narrow window.",
