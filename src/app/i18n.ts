...
const enUS = {
  ...
  helpSecProjectsBody: "Every project you have registered is listed here, with the current one marked. Switching to another project from its row loads that project's workspace and its own enabled functions without a page reload. Only the current project's row offers Edit, Export and Delete — the others offer Switch alone, because only the current project's workspace is held in memory. Deleting a project de-registers it and leaves the underlying file or database untouched. Adding a project opens a three-step wizard (details, template, functions) that can create the new workspace as JSON, CSV or Markdown.",
  helpSecPortfolioHealthTitle: "Portfolio health",
  helpSecPortfolioHealthBody: "A cross-project rollup, available when the storage backend is Turso. It loads each portfolio project's workspace read-only, runs the same health engine the dashboard uses, and shows aggregate indicators above a per-project health table; clicking a row switches to that project. Because it only reads, it is safe to open in a pop-out window. Without a live Turso configuration the view explains what it needs instead of showing figures.",
  helpAutomatedInsightsTitle: "Insights are raised for you",
  helpAutomatedInsightsBody: "The app watches for five patterns and records what it finds: milestone slippage, a worsening overdue trend, stalled work, budget variance, and ageing RAID items. You never create an insight by hand. Each one carries a status — active, acknowledged, acted, dismissed or resolved — so a finding you have already dealt with stops competing for your attention. The dashboard surfaces the active ones; the Insights view is the full log, with filters by status and by type and a toggle that brings resolved and dismissed history back into sight.",
  helpSecTimelogTitle: "Time bookings",
  helpSecTimelogBody: "Pulls booked hours from Timelog and lines them up against this project. People and projects are matched automatically where the names agree, and you confirm or correct the rest by hand. Summary tiles show what was found. The apply step writes matched actuals into your budget buckets and lists exactly which rows would change, so you confirm the effect rather than the intent.",
  helpSecReportsTitle: "Reports",
  helpSecReportsBody: "Read-only summaries, built for printing. Reports covers tasks: totals for open, completed, cancelled and overdue, on-time against late completion, inquiry counts, and breakdowns by group, label, assignee and priority. The RAID report breaks the register down by severity, status, owner, category and age. Change and Budget have their own reports on the same pattern. Nothing on these views edits anything — every table sorts, its columns resize, and a text filter narrows the rows.",
  helpSecHelpTitle: "Finding your way around Help",
  helpSecHelpBody: "This view. Its tabs are Help itself, How it all connects and Information flows, with Guided tours added wherever a tour can actually be launched from. The search box filters entries by title and body together, so a word you remember from a description finds the entry even when the heading does not contain it. Beside it sits the reading level: Guided adds a plain-language primer to each concept, Standard is the default, and Expert puts the reference sections ahead of the explanatory ones. It writes the same device-wide setting as Settings, Appearance, so changing it in either place moves the other. Print produces the whole set.",
  helpSecSavedViewsTitle: "Saved views",
  helpSecSavedViewsBody: "A named preset of how you have set a list up: its search text, its filters, the sort column and direction, and which columns are hidden. Save one, then switch between them from the same menu. Open Points, RAID, Changes, Milestones, Stakeholders and Reports each keep their own set. A view stores the filter values you chose rather than the rows they matched, so a preset preserves your intent even as the data behind it changes.",
  ...
  helpSecUndoBody: "Ctrl+Z (Cmd+Z on a Mac) reverses your last change; Ctrl+Shift+Z or Ctrl+Y replays it. Inside a text field the browser's own undo wins, so typing behaves the way you expect. Changes are captured per group of related fields rather than per keystroke, so one edit that touched both the dates and the owner comes back as two steps. The stack has a fixed depth and drops the oldest entries once it is full. Pop-out windows capture nothing — make edits in the main window if you want to be able to take them back.",
  helpSecInlineAiEditTitle: "Ask Claude to edit one record",
  helpSecInlineAiEditBody: "Tasks, RAID items, changes, milestones and stakeholders can be edited by describing the change in plain language, without leaving the row. The model answers with a plan rather than an action: you see each field it proposes to touch with its current and its new value, and nothing is written until you confirm. You can send a correction instead of confirming. Where a field holds formatted text the preview is shown as plain text while the value actually stored keeps its formatting.",
  helpSecDigestTitle: "Weekly status digest",
  helpSecDigestBody: "A written status summary assembled from the dashboard's own figures rather than from a separate calculation, so the two cannot disagree. It is off until you switch it on and the cadence is yours to choose. It states where the project stands and what has changed since the previous digest, and it can be sent on by email.",
  helpSecTableColumnsTitle: "Column widths",
  helpSecTableColumnsBody: "Drag the divider in a table header to set a column's width, and use the toolbar's reset control to put every column back. Widths are stored per table on this device, and only the columns you actually dragged are recorded — so a later improvement to a default width can still reach you instead of being frozen at the value you happened to see first.",
