...
const enUS = {
  ...
  helpAutomatedHealthBody: "The app derives your project's Overall, Schedule, Budget, and Scope health ratings — red, amber, or green — directly from your tasks, milestones, budget, and change data, so you never have to fill in a status field by hand. Earned-value indicators like the Schedule Performance Index and Cost Performance Index update automatically as work progresses, and the Budget burn-down chart tracks planned spend against actual spend to date. On a Turso database backend the app also takes periodic progress snapshots that feed the Trends view, letting you see how your project has been tracking over time.",
  ...
  versionHighlightIconSet: "Every icon in the app now comes from a single icon set. Two sets shipped side by side before, so the same idea could be drawn two different ways depending on which screen you were on. Nothing should look different: the previous set's line weight is preserved, and each icon was matched to its closest equivalent by eye rather than by name — two of them turned out to draw something else entirely under the same name, and were corrected.",
