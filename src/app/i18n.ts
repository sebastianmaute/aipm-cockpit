...
const enUS = {
  ...
  helpSecAiBody:
    "Add an Anthropic API key in Settings → AI, then accept the consent screen on the AI Assistant tab. Messages and task data are sent directly from your browser to api.anthropic.com — Acme's AI usage policy applies. The key is stored in this browser's localStorage. Claude can list, create, update, and delete tasks on your behalf via tool calls. Claude is also told which view you are on and what that view is for, and on Open Points, Workload, Gantt and Budget what your current filters are showing — so ask about “what I am looking at” and it answers from the screen. Settings → AI lists, read-only, exactly what it is told about each view.",
