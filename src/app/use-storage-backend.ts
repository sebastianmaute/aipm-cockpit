...
export function useStorageBackend(args: UseStorageBackendArgs) {
  ...
  const { truncation, decodeFailureCount, loadWasIncomplete, allowIncompleteSave, mayCommitAfterIncompleteLoad, truncationOps } = useLoadTruncation(langRef, emitToast, () => backend.save(currentWorkspace())); // ★ `emitToast`/`currentWorkspace` are hoisted function declarations; the closure is rebuilt every render, so it always writes the LIVE workspace to the CURRENT backend.
  ...
  return {
    ...
    reloadCurrentProject, allowDestructiveSave, truncation, decodeFailureCount, loadWasIncomplete, allowIncompleteSave,
