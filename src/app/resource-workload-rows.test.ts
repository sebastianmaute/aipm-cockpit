...
describe("buildResourceWorkload", () => {
  ...
  it("falls back to case-folded name when a task has no resourceId", () => {
    const { managed } = buildResourceWorkload([res(1, "Sofia", "Ramirez")], [task(10, "sofia ramirez")], [], [], [], "2026-06-01");
    expect(managed[0].openCount).toBe(1);
  });
  it("counts overdue open tasks", () => {
    const { managed } = buildResourceWorkload([res(1, "Sofia", "Ramirez")], [task(10, "Sofia Ramirez", { resourceId: 1, dueDate: "2026-01-01" })], [], [], [], "2026-06-01");
  ...
  it("carries the overdue task objects for inline triage (#24)", () => {
    const { managed } = buildResourceWorkload(
      [res(1, "Sofia", "Ramirez")],
      [
        task(10, "Sofia Ramirez", { resourceId: 1, dueDate: "2026-01-01" }),
        task(11, "Sofia Ramirez", { resourceId: 1, dueDate: "2099-01-01" }), // not overdue
      ],
      [], [], [], "2026-06-01",
    );
  ...
  it("ignores completed tasks for open/overdue counts", () => {
    ...
    const { managed } = buildResourceWorkload([res(1, "Sofia", "Ramirez")], [task(10, "Sofia Ramirez", { resourceId: 1, dueDate: "2026-01-01", status: "Done", completedDate: "2026-02-01" })], [], [], [], "2026-06-01");
  ...
  it("a cancelled task adds no open or overdue load", () => {
    ...
    const { managed } = buildResourceWorkload(
      [res(1, "Sofia", "Ramirez")],
      [task(10, "Sofia Ramirez", { resourceId: 1, status: "Cancelled", dueDate: "2026-01-01" })],
      [], [], [], "2026-06-01",
    );
    expect(managed[0].openCount).toBe(0);
    expect(managed[0].overdueCount).toBe(0);
  ...
  it("attaches a shift's weekly hours by name match", () => {
    ...
    const { managed } = buildResourceWorkload([res(1, "Sofia", "Ramirez")], [], [], [shift], [], "2026-06-01");
  ...
  it("attaches a shift by resourceId even when the assignee name differs", () => {
    ...
    const { managed, unlinked } = buildResourceWorkload([res(1, "Sofia", "Ramirez")], [], [], [shift], [], "2026-06-01");
    expect(unlinked).toHaveLength(0);
    expect(managed[0].shift).not.toBeNull();
    expect(managed[0].weeklyHours).toBe(32);
  });
  it("collects only upcoming absences within the 60-day horizon", () => {
    const mk = (id: number, s: string, e: string): Absence => ({ id, assignee: "Sofia Ramirez", startDate: s, endDate: e, type: "vacation", resourceId: 1 });
    const { managed } = buildResourceWorkload([res(1, "Sofia", "Ramirez")], [], [mk(1,"2026-01-01","2026-01-05"), mk(2,"2026-06-10","2026-06-12"), mk(3,"2026-12-01","2026-12-05")], [], [], "2026-06-01");
...
describe("buildResourceWorkload — raidOpenCount", () => {
  it("counts open RAID a resource owns, by ownerResourceId and by owner name", () => {
    const r = res(1, "Sofia", "Ramirez");
    const raid = [raidItem(10, { ownerResourceId: 1 }), raidItem(11, { owner: "sofia ramirez" })];
  ...
  it("buckets a free-text RAID owner with no match under unlinked", () => {
    ...
    const { managed, unlinked } = buildResourceWorkload([res(1, "Sofia", "Ramirez")], [], [], [], raid, "2026-06-01");
    expect(managed[0].raidOpenCount).toBe(0);
    expect(unlinked).toHaveLength(1);
