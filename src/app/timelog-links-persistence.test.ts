...
describe("use-storage-backend autosave wiring — timelogLinks", () => {
  ...
  // timelogLinks — covers the `outgoing` snapshot, the currentWorkspace()
  ...
  // re-spell the tuple — otherwise `timelogLinks` can be present in all three
  // lines above and still be dropped by a write path one file over, which is
  // the exact defect this scan exists to catch.
