...
// ★★ The structural injection-guard regex here MIRRORS scheme-apply's
//    isSafeRawCssValue (charset allowlist + dangerous-substring denylist).
...
export const NO_FLASH_THEME_SCRIPT = `(function(){try{var st=localStorage.getItem("lop-style");var legacyIcc=st==="AIPM";var legacyMockup=st==="mockup";var isCustom=st==="custom";document.documentElement.setAttribute("data-style","custom");var t=localStorage.getItem("lop-theme")||"system";var themeDark=t==="dark"||(t==="system"&&window.matchMedia("(prefers-color-scheme: dark)").matches);var rawC=localStorage.getItem("lop-active-scheme-colors");var rawS=localStorage.getItem("lop-active-scheme-structural");var schemeDark,colors,structural;if(legacyMockup){schemeDark=false;colors=rawC?JSON.parse(rawC):${MOCKUP_LIGHT_COLORS};structural=rawS?JSON.parse(rawS):${MOCKUP_STRUCTURAL_JSON};}else if(legacyIcc){schemeDark=true;colors=rawC?JSON.parse(rawC):(themeDark?${ICC_DARK_COLORS}:${ICC_LIGHT_COLORS});structural=rawS?JSON.parse(rawS):${ICC_STRUCTURAL_JSON};}else if(isCustom){schemeDark=localStorage.getItem("lop-scheme-supports-dark")==="1";colors=rawC?JSON.parse(rawC):((themeDark&&schemeDark)?${HARBOR_DARK_COLORS}:${HARBOR_LIGHT_COLORS});structural=rawS?JSON.parse(rawS):{};}else{schemeDark=true;colors=themeDark?${HARBOR_DARK_COLORS}:${HARBOR_LIGHT_COLORS};structural={};}var d=themeDark&&schemeDark;document.documentElement.setAttribute("data-scheme-dark",schemeDark?"1":"0");document.documentElement.classList.toggle("dark",d);var el=document.documentElement;for(var k in colors){if(Object.prototype.hasOwnProperty.call(colors,k)&&typeof colors[k]==="string"&&/^--[\\w-]+$/.test(k)&&/^#[0-9a-fA-F]{3,8}$/.test(colors[k])){el.style.setProperty(k,colors[k]);}}for(var j in structural){if(Object.prototype.hasOwnProperty.call(structural,j)&&typeof structural[j]==="string"&&/^--[\\w-]+$/.test(j)&&/^[\\w\\s#.,%()/-]+$/.test(structural[j])&&!/url\\(|expression|image-set|[;{}@<>\\\\]/i.test(structural[j])){el.style.setProperty(j,structural[j]);}}}catch(e){}})();`;
