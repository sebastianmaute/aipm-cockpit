...
export const APP_BUILD_DATE = "2026-09-10"; // 0.301.0: the help icon now reaches four surfaces that have no dialog header to hang it on — the RACI suggestion dialog, the meeting-series and task-effort editors, and the floating note-log window — and three new Help entries stand behind them, covering recurring meetings in the resource calendar, images inside documents, and how a task's effort is estimated and rolled up; twelve further dialogs were examined and deliberately left without an icon, each with its reason recorded, because an icon that opens an entry describing a different surface is worse than no icon at all (Arnason)
...
// Bump BOTH together.
export const APP_MILESTONE = "Arnason";
