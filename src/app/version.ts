...
export const APP_BUILD_DATE = "2026-09-05"; // 0.283.0: when the assistant is about to delete something, or to change more than one row at once, it now shows you the plan first — every row with what it would change — and writes nothing until you approve it; the rows you keep are applied as ONE step you can undo in one press (Lessing)
 ...
 *  0.283.x line is "Lessing" (Doris Lessing, British-Zimbabwean Nobel
 *  laureate, whose "Canopus in Argos" sequence is her science fiction).
 ...
 *  `grep -ic lessing CHANGELOG.md` returns 0, in a run whose positive controls
 *  all fired — `cadigan` 2, `sinisalo` 1, `zamyatin` 1, and `banks` 1, that
 *  last one being the header shape an END-OF-LINE anchor could not see. Two
 *  further candidates cleared the same sweep and were not needed
 *  (`ballantyne`, `piper`).
...
// Bump BOTH together.
export const APP_MILESTONE = "Lessing";
