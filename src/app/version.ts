...
export const APP_BUILD_DATE = "2026-09-21"; // 1.13.0: a Dashboard and branding release — Dashboard tiles size themselves to their content, the Progress tile is merged into At a glance with its trend arrows beside each number, there is one status summary edited in place, and the Completion trend is dated, drawn on a true scale and hidden on a fresh board and after Reset layout; Reports opens with the same At a glance strip; the AI usage policy on the assistant's consent screen and the footer of HTML, print/PDF and PowerPoint exports are configurable (Settings, or NEXT_PUBLIC_AI_POLICY_ORG/_URL for the policy), defaulting to the familiar Acme text; a closed budget bucket with no percent complete counts as 100 %; and a load-and-save patch — the AI assistant no longer finishes a turn into whichever project is in scope when each tool comes up: a turn left running while you switch project, or change where the project is stored, now has its remaining writes dropped rather than landing in the project that replaced it (§596, and a staged plan applied from the review card is not yet guarded the same way — §600); a chat turn whose tools were dropped no longer leaves the conversation in a state that fails every later message, which previously could only be escaped by starting a new thread; leaving the assistant by opening another view no longer silently cancels the work you just asked for; a reload or file pick left running from before a storage change no longer shuts the new target's save gate (§588) and an edit made in the moment before that change is no longer dropped (§589); "Pick storage file" after a failed load no longer writes an empty workspace over the file you chose (§590); and the demo project now shows Effort SPI and CPI on the dashboard's At-a-glance tile, which had been blank since the tile shipped (Connelly)
...
// 1.12.3: a test-infrastructure patch, no production code changes — every timed unit test now judges the median of per-pair ratios from expectLinearScaling (n vs 4n) against a limit of 8 instead of a wall-clock ceiling, so machine load can no longer fail a correct build while a real quadratic regression still fails at a ratio of about 16; 22 call sites across 9 test files were converted, each guard proven against its historical regression at a ratio of at least 12 (§592, §593); the document-asset-patterns "one huge tag" row is filed open with no known regression to prove its guard against (§594) (Child)
// 1.12.2: a data-loss patch — applying an assistant proposal now writes exactly the fields its review card showed, leaving out a refused field instead of losing the whole change, and an insight recommendation whose changes are all refused says nothing was applied (§534); the TimeLog Apply dialog lists the hand-typed hours a dated Apply removes from the other granularity's period total (§546); and every sealed secret id is read back from the one SECRET_IDS list, so an id missing from a hand-kept copy can no longer be dropped on read (§567) (Child)
// 1.12.1: a data-loss hotfix — no save of any kind reaches a backend until a load for that backend has been applied, so the startup autosave can no longer write the still-empty workspace over the stored project (§586) and a Turso URL/token edit or a SharePoint target change can no longer save the open project into the new target before its own data loads (§587); a pause caused by a failed or empty load now stays on the saving-paused banner, a storage-type switch made before anything loaded switches without copying, and §588–§590 are filed open (Child)
 ...
 *  `grep -ciw connelly CHANGELOG.md` 0, `grep -c '"Connelly"' CHANGELOG.md` 0,
 *  `git log --all --format=%s | grep -ciw connelly` 0 and 0 hits in `src`;
 *  positive control `grep -c '"Child"' CHANGELOG.md` 8, negative control
 *  `zzznotaname` 0 — all measured on 1.13.0's parent; on 1.13.0 itself "Child" reads 7, because
 *  the release renamed the 1.12.8 "Child" header. 1.13.0 was prepared as the 1.12.8 patch and became a minor
 *  release when the Dashboard, Reports and branding work joined it.
 *  The 1.12.x line was "Child" (Lee Child, British thriller novelist, author of
