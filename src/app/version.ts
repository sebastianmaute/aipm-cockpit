...
export const APP_BUILD_DATE = "2026-09-12"; // 1.0.2: every money figure the budget engine produces is EUR and the surfaces say so — a fixed-price bucket held in another currency is converted before it is compared against its cost, where before the contract amount was read as if it were EUR and the bucket reported a healthier margin than it had; an FX rate stored against a EUR bucket is now ignored rather than divided by, which repairs buckets that kept a stale rate after their currency was switched back and had been shrinking an 80,000 EUR contract to 72,727 on the project rollup and throughout the Budget report; four budget surfaces now say EUR instead of the plan's symbol, the money ratio is no longer called CPI (which is EVM's term for a different number), and three hints are corrected in English and German — a non-EUR PLAN stays unsupported and its margin is now wrong where it used to be right, filed as follow-up 473 (Pratchett)
