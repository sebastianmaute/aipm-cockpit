 ...
 *  1.11.x line is "Tey" (Josephine Tey, Scottish crime novelist, author of
 *  "The Franchise Affair", 1948, and "The Daughter of Time", 1951) — provided
 *  directly rather than drawn from the SELECTION procedure's candidate list:
 *  it is absent from BOTH sections of `docs/release-codenames.md`, so this is
 *  an explicit pick outside the usual procedure, the same shape as "Hodgell"
 *  for the 1.2.x line below; the candidate list is left untouched and "Tey"
 *  joins the front of the "Already Used" list. Swept BEFORE the 1.11.0 header
 *  was written, in one run with its controls: `grep -ciw tey CHANGELOG.md` 0,
 *  `grep -c '"Tey"' CHANGELOG.md` 0, and `git log --format=%s | grep -ciw
 *  tey` 0; positive control `grep -c '"Leonard"' CHANGELOG.md` 2, negative
...
// Bump BOTH together.
export const APP_MILESTONE = "Tey";
