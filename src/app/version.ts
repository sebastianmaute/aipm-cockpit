...
export const APP_BUILD_DATE = "2026-09-25"; // 1.14.0: the first release built and published by GitHub Actions — the desktop app updates itself from GitHub Releases, asking before it downloads and again before it restarts; the Version panel links to GitHub Sponsors; SECURITY.md says how to report a vulnerability; and the GitLab release pipeline is gone (Deaver)
// 1.14.0-rc.1: a release pipeline rehearsal, not for use — the first build published by the GitHub Actions release workflow, as a prerelease that the desktop auto-updater ignores; it also ships ask-then-install auto-update from GitHub Releases (Deaver)
...
// 1.4.0: the Version panel shows a one-line pitch and five highlights of what makes the app different instead of a per-release history, and its employer footer gives way to the EUPL-1.2 licence, an author line built with Claude Code and links to GitHub and LinkedIn; the start window shows the transparent beacon banner, the classic header logo is 300×70 and the modern sidebar logo spans the sidebar; a guardrail insight's person link opens that person's editor in the Resources directory; Outlook Push and Pull buttons name their entity; the chat attach button no longer clicks a display:none input; a Kanban task with no resource link lands in its person's lane by email, never an external's; and the Gantt no longer numbers a milestone it cannot draw — repeating a person link while that person's editor is open re-runs the open, filed as follow-up 540 (Hammett)
