...
export const APP_BUILD_DATE = "2026-09-15"; // 1.5.0: TimeLog actuals keep their booking days — Apply writes each person's hours per booking day into a budget line and replaces a covered period whole, the budget report, burn-down and Budget view count those days into whichever month or week holds them, so switching the plan between monthly and weekly after a fetch or an Apply no longer strands applied hours under keys the report never reads (follow-up 169); a period holding TimeLog day hours is read-only in the Budget view; re-applying the same bookings changes nothing, even for hours with more than two decimals; and the per-device TimeLog cache packs its day cells so a large project fits, with a warning in the TimeLog panel when the browser still refuses the save — an older build drops day keys on load, so do not roll back past this release once TimeLog hours are applied, and a period key of the other granularity is counted twice after switching back, filed as follow-up 543 (Highsmith)
 ...
 *  1.5.x line is "Highsmith" (Patricia Highsmith, American crime novelist, author
...
// Bump BOTH together.
export const APP_MILESTONE = "Highsmith";
