...
export const APP_BUILD_DATE = "2026-09-11"; // 1.0.0: AI PM Cockpit's first release published as an installer package — it replaces the 0.303.0 GitLab Release, which is being withdrawn along with its tag; the Windows installer no longer carries sharp's Linux binaries that a Linux CI runner had been installing into the bundled server though the app never loads them, freeing an estimated ~6 MiB against GitLab's artifact size limit, with the desktop-package CI jobs now failing if a sharp package reappears; and a spike write-up records what the first CI builds and the first GitLab Release actually measured — image pull times, artifact sizes and contents, and that a signed-in non-member can download the installer — leaving a new open follow-up for a Linux installer (Pratchett)
