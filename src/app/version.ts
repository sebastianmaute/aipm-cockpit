...
export const APP_BUILD_DATE = "2026-09-21"; // 1.12.7: a backlog-sweep patch — the TimeLog cap threshold field accepts a fractional value (§365); Enter-submitting the stakeholder editor caps and trims its text fields the same way blur does (§541); a calendar event's date fields no longer accept a day past its month's end and roll it over (§542); a calendar-invalid TimeLog day such as 2026-02-30 no longer lands in a real month by one reading and a different real ISO week by another (§544); diagnostics redaction now catches an opaque token that mixes case and a digit with no vendor prefix (§564); four settings sections now remove a cleared secret instead of resealing an empty string (§565); the Jira proxy no longer logs the raw fetch-rejection object server-side (§566); the chart readout's spoken tip no longer reads as a capitalised word mid-sentence (§570); the chart-box clamp now fires at exactly its boundary width (§571); an FX rates snapshot now round-trips through JSON byte-stable (§576); and §391 is closed as a record, correcting a stale review-card comment about which staged tools the descriptor engine cannot diff (Child)
