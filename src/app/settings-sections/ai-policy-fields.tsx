...
import { MAX_AI_POLICY_FIELD, isSafePolicyUrl, resolveAiPolicy } from "../ai-policy";
 ...
 *   nothing. ★ There is no built-in default: a never-set field and a deliberately
 *   cleared one ("") both render empty, and the consent screen shows no policy
 *   step until both an owner and a link are entered.
 */
export function AiPolicyFields({
  ...
  const orgValue = ai.policyOrgName ?? "";
  // ★ A never-set link shows what the consent screen will actually use — nothing,
  //   unless a build variable or a stored value supplies one.
  const urlValue = ai.policyUrl ?? policy.url ?? "";
  const urlInvalid = urlValue.trim() !== "" && !isSafePolicyUrl(urlValue);
  // ★★ Say WHY the link is blank: no link stored and none from the deployment
  //   environment — on a consent surface that must never happen silently.
  const builtinLinkDropped = !policy.urlFromEnv && ai.policyUrl === undefined;
