...
/** The typed email judged as the value that would be STORED (`sanitizeEmail`:
 *  trim + EMAIL_MAX cap) with the write rule and no stored value. A blank or
 *  refused value yields undefined: the resource is created WITHOUT an email,
 *  and the host editor's own field still refuses the typed value on save. */
export function creatableResourceEmail(typed: string): string | undefined {
  const value = sanitizeEmail(typed);
