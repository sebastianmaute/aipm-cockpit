...
describe("RichTextEditor", () => {
  ...
  // heading toolbar buttons and a sanitizer that allows h1/h2, so disabling the
  // markdown shortcut here would be a regression, not a fix.
...
describe("RichTextEditor lean variant", () => {
  ...
  // ★★ The lean sanitizer's allow-list has no h1-h6/blockquote/pre and drops a
  // disallowed block's TEXT with it, so a markdown input rule used to turn the
  // whole paragraph into "" on commit while the editor kept showing it.
  it.each([
    ...
    async (_name, typed, visible, kept) => {
      ...
      expect(surface.querySelector("h1, h2, h3, blockquote, pre")).toBeNull();
