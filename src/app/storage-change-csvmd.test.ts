...
const change: ChangeItem = {
  ...
  impact: "High", raisedDate: "2026-06-01", linkedTaskIds: [3], linkedRaidIds: [7], stakeholderIds: [3],
};
...
describe("changes CSV round-trip", () => {
  it("survives CSV write/read", () => {
    ...
    expect(back.changes?.[0]).toMatchObject({ id: 1, type: "Scope", status: "Approved", linkedTaskIds: [3], linkedRaidIds: [7], stakeholderIds: [3] });
  });
});
...
describe("changes Markdown round-trip", () => {
  it("survives Markdown write/read", () => {
    ...
    expect(back.changes?.[0]).toMatchObject({ id: 1, title: "Widen scope", linkedRaidIds: [7], stakeholderIds: [3] });
  });
});
