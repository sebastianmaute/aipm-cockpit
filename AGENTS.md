...
npm run dup:check           # jscpd duplication GATE (--threshold 2.4, per-format; BLOCKING in CI). baseline docs/baselines/jscpd-2026-07.json
...
- **CI is GitLab** (not GitHub),  (GitLab). Pipeline: install → quality (lint · typecheck · **semgrep** SAST
  ...
  BLOCKING [jscpd `--threshold 2.4`, per-format] · **unit** [coverage floors: global lines 92/funcs 91/branch
