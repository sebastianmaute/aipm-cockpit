...
[![version](https://img.shields.io/badge/version-v0.290.0_%22Holdstock%22-2e7d32)](./CHANGELOG.md)
...
| `npm run followups:index:check` | Fail if docs/open-followups.md's index table disagrees with its `## <n>.` headings — a heading with no row, or a row with no heading (BLOCKING; exit 1 = drift, exit 2 = the gate could not scan at all) |
