...
[![version](https://img.shields.io/badge/version-v0.270.0_%22Tchaikovsky%22-2e7d32)](./CHANGELOG.md)
...
| Documents | Author project documents in the app and export each one as a web page, Word, PowerPoint or PDF.<br><details><summary>Details</summary>A document is structured content, not a stored file — the bytes are rendered on demand, so no blob is ever carried in the project. It can **embed a live data section** that renders from the project's current records at export time using the same builder the workspace exporter uses, so an embedded table cannot drift from the export. It can also **name the parts of the project it is about** (tasks, RAID items, milestones, stakeholders); if one of those is later deleted the reference keeps the name it had. Every write keeps a **before-image**, so a per-row History button shows earlier states and restores one verbatim, and a deleted document stays recoverable from a show-deleted toggle while its history is retained. Documents are saved on every storage backend. A project that exceeds the document size cap discloses the truncation rather than silently dropping the excess, and pauses saving so a truncated load cannot overwrite what is on disk.</details> |
...
| Formatted descriptions & note log | Descriptions across the registers take real formatting from one shared editor, and tasks, RAID items and changes each carry a dated note log in a floating window.<br><details><summary>Details</summary>Seven fields take formatting — a task's description, a RAID item's description and mitigation, a change's description, impact and resolution notes, and a milestone's description. One editor serves all seven: **bold, italic, underline, strikethrough, inline code and code blocks, highlight, superscript and subscript, headings, quotes, rules, bulleted, numbered and checkable task lists, left/centre/right alignment, and links**, with Markdown shortcuts while you type. Character counters measure the **visible text**, so markup never eats into your limit. Tasks, RAID items and changes also keep a **note log**: dated entries in a shared draggable window (also reachable as "Notes (N)" from the editor), with a `🗒 N` badge on the row. Note authorship is per-device and honour-system — set who you are in Settings, there is no sign-in — and you can edit or delete your own entries. Notes are write-through: an entry added while an editor is open is never lost when that editor saves.<br><br>**In exports**, headings, list structure and paragraph alignment survive into Word, HTML and PDF rather than flattening; a multi-paragraph description lays out as real paragraphs in XLSX and PPTX rather than one run-on line. CSV and Markdown are the app's own storage format and deliberately carry the description exactly as stored, markup included, so that exporting and re-importing a project round-trips without loss.<br><br>Existing plain-text descriptions keep working untouched — they are upgraded as they are read, not rewritten on disk, so nothing needs migrating and no data is touched to enable this.</details> |
...
| `npm run e2e:crossengine` | Run e2e-crossengine/ in BOTH real Chromium and real Firefox (playwright.crossengine.config.ts, port 3300, workers=1). Covers the popover focus behaviour the unit suite is structurally blind to: jsdom's element-removal focus semantics are Firefox's, so a Chromium-only defect stays green in vitest. Needs Firefox installed (`npx playwright install firefox`) |
