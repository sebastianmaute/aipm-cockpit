...
A full run is **65 requests** — that figure is measured, and every recorded run carries its own request count (a filtered diagnostic run is far smaller). The cost, on the order of **$2** against a mostly-cached prefix, is modelled rather than measured: the app deliberately ships no price table, so nothing in this repo turns a run's token counts into a currency amount. A dry run is the default and spends nothing; spending requires an explicit opt-in, and the harness refuses to spend when it detects a CI environment. It exits **0** on pass, **1** on a real regression, and **2** when it could not do its job — the 1/2 split is deliberate, because a broken measurement must never be reported as a failure of the thing being measured, and a run that measured nothing must never report success.

**Status:** the harness has been run against the live API and carries a recorded baseline in `docs/baselines/ai-eval-runs.json` — read that artifact rather than this sentence, which cannot keep up with it. Its most recent run passes: every probe hit on both arms, the negative control silent, the seeded anchor and the rolling drift reference both clean, and no probe reaching for a tool instead of answering. That pass is an A/A self-test of the machinery and not evidence about a candidate layout, because the candidate arm is still an alias of the current one — it shows the harness measures what it claims to, not that any relocation is safe.
